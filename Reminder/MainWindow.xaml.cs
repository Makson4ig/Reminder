...
namespace Reminder
{
    ...
    public partial class MainWindow : Window
    {
        ...
        {
            InitializeComponent();
            connection = ConfigurationManager.ConnectionStrings["Reminder.Properties.Settings.ConnectionString"].ConnectionString;
        }
        ...
        {
            string localDateTime = DateTime.Now.ToString("dd.MM.yyyy HH:mm:00"); // Шаблон текущего времени 
            MySqlConnection Connect = new MySqlConnection(connection);
            Connect.Open(); // Открываем базу данных 
            var s = Connect.Query<String>(@"select Task+""|""+StartTime from Reminder"); // Присваем переменной массив данных выполненного запроса
        ...
        {
            MySqlConnection Connect = new MySqlConnection(connection);
            MySqlDataAdapter dataAdapter = new MySqlDataAdapter("Select * FROM Reminder ORder BY id", Connect); // Переменная заполняющая DataSet данными из БД.
